...
const LoginForm = (props: any) => {
	...
	const onFinish = async (loginForm: Login.ReqLoginForm) => {
		try {
			...
			loginForm.password = md5(loginForm.password); // 对密码进行 md5 加密
			const { data } = await loginApi(loginForm); // 调用登录 API
			setToken(data?.access_token); // 将获取的 access_token 存入 Redux
			setTabsList([]); // 清空 tabs 列表
			message.success("登录成功！"); // 显示登录成功的消息
			navigate(HOME_URL); // 导航到主页
		} finally {
